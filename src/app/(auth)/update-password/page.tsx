          ...
          <form onSubmit={handleUpdatePassword} className="space-y-5">
            ...
            <div className="bg-muted/50 p-4 rounded-xl text-sm space-y-2">
              ...
              <ul className="space-y-1">
                <li
                  className={`flex items-center gap-2 ${isLengthValid ? "text-green-600" : "text-muted-foreground"}`}
                >
                  <span className="text-lg">{isLengthValid ? "✓" : "○"}</span>{" "}
                  Tối thiểu 8 ký tự
                </li>
                <li
                  className={`flex items-center gap-2 ${hasUpperCase.test(password) ? "text-green-600" : "text-muted-foreground"}`}
                >
                ...
                <li
                  className={`flex items-center gap-2 ${hasLowerCase.test(password) ? "text-green-600" : "text-muted-foreground"}`}
                >
                  <span className="text-lg">
                    {hasLowerCase.test(password) ? "✓" : "○"}
                  </span>{" "}
                  Chứa chữ in thường (a-z)
                </li>
                <li
                  className={`flex items-center gap-2 ${hasNumber.test(password) ? "text-green-600" : "text-muted-foreground"}`}
                >
                  <span className="text-lg">
                    {hasNumber.test(password) ? "✓" : "○"}
                  </span>{" "}
                ...
                <li
                  className={`flex items-center gap-2 ${hasSpecialChar.test(password) ? "text-green-600" : "text-muted-foreground"}`}
                >
                  <span className="text-lg">
                    {hasSpecialChar.test(password) ? "✓" : "○"}
                  </span>{" "}
