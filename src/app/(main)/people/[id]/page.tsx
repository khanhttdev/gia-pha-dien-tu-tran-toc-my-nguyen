...
import { getMemberById, getAllMembers, getAllSpouses } from '@/lib/supabase-data'
import type { Member, Spouse, MemberMetadata } from '@/lib/types'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { Loader2, ArrowLeft, User2, Heart, TreePine, Baby, Star } from 'lucide-react'
...
export default function MemberProfilePage() {
    ...
    const [allSpouses, setAllSpouses] = useState<Spouse[]>([])
    const [loading, setLoading] = useState(true)

    useEffect(() => {
        const load = async () => {
            setLoading(true)
            const [m, members, spouses] = await Promise.all([
    ...
    // ── Helpers ───────────────────────────────────────────────────────────────
    const genderColor = member.gender === 'male' ? 'blue' : 'rose'
        ...
        <div className="h-full overflow-y-auto">
            <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 pb-16 space-y-6 animate-in fade-in-50 duration-500">
                ...
                <div className="glass rounded-2xl border border-border/50 overflow-hidden">
                    ...
                    <div className={cn(
                        "h-1.5",
                        member.gender === 'male' ? "bg-gradient-to-r from-blue-500 to-cyan-400" : "bg-gradient-to-r from-rose-400 to-pink-500"
                    )} />
                        ...
                        <div className="flex flex-col sm:flex-row items-start gap-6">
                            ...
                            <div className="shrink-0">
                                {meta.avatar_url ? (
                                    ...
                                    <div className={cn(
                                        ...
                                        member.gender === 'male' ? "bg-blue-500/10" : "bg-rose-400/10"
                                    )}>
                                        {member.gender === 'male' ? '👨' : '👩'}
                                    </div>
                                )}
                            ...
                            <div className="flex-1 min-w-0">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                                            <Badge variant="outline" className={cn(
                                                "text-xs",
                                                member.gender === 'male' ? "border-blue-500/40 text-blue-500" : "border-rose-400/40 text-rose-400"
                                            )}>
                                                {genderLabel}
                                            </Badge>
                                            ...
                                            {meta.is_alive === false ? (
                                                <Badge variant="outline" className="text-xs border-gray-500/40 text-muted-foreground">
                                                    Đã mất
                                                </Badge>
                                            ) : (
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-5">
                                    ...
                                    <div>
                                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Thứ tự sinh</p>
                                        <p className="font-semibold text-sm mt-0.5">Con thứ {member.birth_order ?? 1}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Số con</p>
                                        <p className="font-semibold text-sm mt-0.5">{children.length} người</p>
                                    </div>
                                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="glass rounded-xl border border-border/50 p-5 space-y-3">
                        ...
                        {father ? (
                            <Link href={`/people/${father.id}`} className="flex items-center gap-3 group">
                                ...
                                <div>
                                    <p className="text-sm font-medium group-hover:text-amber-500 transition-colors">{father.full_name}</p>
                                    <p className="text-xs text-muted-foreground">Cha · Thế hệ {father.generation_level}</p>
                                </div>
                            </Link>
                        ) : (
                            <p className="text-xs text-muted-foreground italic">Không có thông tin cha</p>
                        )}
                        {mother ? (
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">Mẹ · {roleLabel(mother.role_type ?? '')}</p>
                                </div>
                            </div>
                        ) : (
                            <p className="text-xs text-muted-foreground italic">Không có thông tin mẹ</p>
                        )}
                    </div>
                ...
                {children.length > 0 && (
                    <div className="glass rounded-xl border border-border/50 p-5 space-y-3">
                        <h2 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                            <Baby className="w-3.5 h-3.5 text-emerald-500" /> Con Cái ({children.length})
                        </h2>
                                ...
                                return (
                                    ...
                                    >
                                        <div className={cn(
                                            ...
                                            child.gender === 'male' ? "bg-blue-500/10 border-blue-500/30" : "bg-rose-400/10 border-rose-400/30"
                                        )}>
                                            {child.gender === 'male' ? '👦' : '👧'}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {childMeta.birth_year ? `Sinh ${childMeta.birth_year}` : `Con thứ ${child.birth_order ?? 1}`}
                                            </p>
                                        </div>
                                        {childMeta.is_alive === false && (
                                            <Badge variant="outline" className="text-[10px] shrink-0">Đã mất</Badge>
                                        )}
                                    </Link>
                                )
                            })}
                ...
                {siblings.length > 0 && (
                    <div className="glass rounded-xl border border-border/50 p-5 space-y-3">
                        <h2 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                            <User2 className="w-3.5 h-3.5 text-sky-400" /> Anh Chị Em ({siblings.length})
                        </h2>
                        <div className="flex flex-wrap gap-2">
                            {siblings.map(s => (
                                ...
                                >
                                    ...
                                    <span className="group-hover:text-amber-500 transition-colors">{s.full_name}</span>
                                </Link>
                            ))}
                        </div>
                    </div>
