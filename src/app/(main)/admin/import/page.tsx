...
export default function AdminImportPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setFileData(parsed);
      toast.success(
        `Đã phân tích file: ${parsed.individuals.length} thành viên, ${parsed.families.length} gia đình.`,
      );
    } catch (error: any) {
      console.error("Lỗi khi đọc file GEDCOM:", error);
      toast.error(
        "Không thể đọc file. File bị lỗi cú pháp hoặc không đúng chuẩn GEDCOM.",
      );
    } finally {
      setIsParsing(false);
    }
    ...
    <div className="p-6 max-w-2xl mx-auto w-full">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-2">
          Nhập dữ liệu gia phả (Import)
        </h1>
        <p className="text-sm text-muted-foreground">
          Nhập tệp dữ liệu chuẩn quốc tế chuẩn `.ged` (GEDCOM) xuất ra từ các
          phần mềm quản lý gia phả khác. Hệ thống sẽ tự động tìm các bản ghi
          trùng lặp và bỏ qua nếu thành viên đã có trong Hệ thống.
        </p>
      </div>
      ...
      <div className="glass rounded-xl border border-border/50 p-8 flex flex-col items-center justify-center text-center space-y-4 mb-8">
        ...
        <p className="text-sm text-muted-foreground max-w-md">
          Kéo thả tệp hoặc bấm vào nút bên dưới để tải lên tệp gia phả của bạn.
          Chỉ hỗ trợ tiếng Anh hoặc tiếng Việt Unicode.
        </p>
      ...
      {importResult && (
        <div
          className={`p-4 rounded-lg flex items-start gap-3 mt-4 border ${importResult.success ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-600" : "bg-red-500/10 border-red-500/20 text-red-600"}`}
        >
