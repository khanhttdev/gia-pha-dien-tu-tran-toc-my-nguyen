...
export async function GET(request: Request) {
    ...
    const [membersRes, spousesRes, eventsRes, postsRes] = await Promise.all([
        supabase.from('members').select('id, full_name, generation_level, gender').ilike('full_name', `%${q}%`).limit(5),
        supabase.from('spouses').select('id, full_name, member_id, role_type').ilike('full_name', `%${q}%`).limit(5),
        supabase.from('events').select('id, title, event_date').ilike('title', `%${q}%`).limit(5),
        supabase.from('contributions').select('id, content, type').ilike('content', `%${q}%`).eq('status', 'approved').limit(5)
    ])
