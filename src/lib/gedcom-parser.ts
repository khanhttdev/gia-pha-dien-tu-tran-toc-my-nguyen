...
export function parseGedcom(gedcomText: string): GedcomData {
    ...
    for (const line of lines) {
        ...
        if (level === 0) {
            ...
            else if (tag === 'SEX') indi.gender = value === 'M' ? 'male' : value === 'F' ? 'female' : 'unknown';
            ...
            if (tag === 'DATE') {
                ...
                if (yearMatch) {
                    if (currentContext === 'BIRT') indi.birthYear = parseInt(yearMatch[1]);
                    if (currentContext === 'DEAT') indi.deathYear = parseInt(yearMatch[1]);
                }
            }
