...
import { Member, MemberInsert, MemberUpdate, Spouse, SpouseInsert, SpouseUpdate } from './types'
...
export async function getSpousesByMemberId(memberId: string): Promise<Spouse[]> {
    const { data, error } = await db()
        .from('spouses')
        .select('*')
...
export async function updateSpouse(id: string, updates: SpouseUpdate): Promise<Spouse> {
    const { data, error } = await db()
        .from('spouses')
        .update({ ...updates, updated_at: new Date().toISOString() })
        .eq('id', id)
...
export const updatePerson = updateMember as (id: string, updates: any) => Promise<Member>
