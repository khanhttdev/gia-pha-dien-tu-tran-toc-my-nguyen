...
export async function importGedcomAction(data: GedcomData) {
    ...
    for (const indi of data.individuals) {
        ...
        const query = supabase.from('members').select('id, metadata').eq('full_name', indi.name).limit(1)
    ...
    for (const fam of data.families) {
        ...
        for (const childId of fam.childrenIds) {
            ...
            if (childUuid && husbandUuid) {
                await supabase.from('members').update({ father_id: husbandUuid }).eq('id', childUuid)
            }
        }
        ...
        if (husbandUuid && wifeUuid) {
            ...
            if (wifeIndi) {
                ...
                const { data: existingSpouse } = await supabase.from('spouses').select('id')
