  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass rounded-xl p-6 border border-amber-500/20 text-center space-y-2">
        ...
        <p
          className={cn(
            "text-4xl font-black font-playfair",
      ...
      <form
        onSubmit={handleAdd}
        className="glass rounded-xl p-4 border border-border/50 flex flex-wrap gap-4 items-end"
      >
        ...
        <Button
          type="submit"
          disabled={isAdding}
          className="h-9 gap-1.5 shrink-0 bg-amber-600 hover:bg-amber-700 text-white"
        >
          {isAdding ? (
      ...
      <div className="space-y-3">
        ...
        {funds.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">
            Chưa có giao dịch quỹ nào được ghi nhận.
          </p>
        ) : (
          <div className="space-y-2">
            {funds.map((f: any) => (
              <div
                key={f.id}
                className="glass rounded-xl p-3 border border-border/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 transition-colors hover:bg-white/5"
              >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <p className="text-xs text-muted-foreground">
                        {new Date(f.transaction_date).toLocaleDateString(
                          "vi-VN",
                        )}
                      </p>
                      {f.member && (
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0 pt-2 sm:pt-0 border-t border-border/40 sm:border-0 border-dashed">
                  ...
                  <Button
                    ...
                    size="icon"
                    variant="ghost"
                    className="h-8 w-8 text-blue-500 hover:text-blue-600 hover:bg-blue-50/10"
                    onClick={() => startEdit(f)}
                  >
      ...
      <Dialog open={!!editingTx} onOpenChange={(o) => !o && setEditingTx(null)}>
        <DialogContent className="sm:max-w-[425px] glass border-amber-500/20">
          <DialogHeader>
            ...
            <DialogDescription>
              Chỉnh sửa này sẽ được ghi nhận vào nhật ký quỹ để đảm bảo minh
              bạch.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <AlertDialogContent className="glass border-red-500/20 sm:max-w-[400px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Hành động này không thể hoàn tác. Số dư quỹ sẽ tự động cộng/trừ
              lại dựa trên thay đổi này. Lịch sử xóa sẽ được lưu vào nhật ký hệ
              thống.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
