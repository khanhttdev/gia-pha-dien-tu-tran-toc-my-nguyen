...
export function CommentSection({ contributionId, currentUserId }: CommentSectionProps) {
        ...
        <div className="mt-4 pt-4 border-t border-border/40 space-y-4 animate-in slide-in-from-top-2 duration-300">
            ...
            <div className="space-y-3">
                {loading && comments.length === 0 ? (
                    ...
                    <p className="text-[11px] text-muted-foreground italic px-2">Chưa có bình luận nào. Hãy bắt đầu cuộc trò chuyện!</p>
                ) : (
                    comments.map((comment) => (
                        <div key={comment.id} className="flex gap-2 group">
                            <div className="shrink-0">
                                {comment.author?.avatar_url ? (
                                    <img src={comment.author.avatar_url} alt="avt" className="w-6 h-6 rounded-full object-cover" />
                                ) : (
                            ...
                            <div className="flex-1 bg-secondary/30 rounded-2xl px-3 py-2">
                                <div className="flex items-center justify-between gap-2">
                                    <span className="text-[11px] font-bold text-foreground/80">{comment.author?.full_name || 'Thành viên'}</span>
                                    <span className="text-[9px] text-muted-foreground">{new Date(comment.created_at).toLocaleDateString('vi-VN')}</span>
                                </div>
            ...
            {!currentUserId && (
                <p className="text-[10px] text-muted-foreground text-center">Vui lòng đăng nhập để bình luận.</p>
            )}
        </div>
    )
}
