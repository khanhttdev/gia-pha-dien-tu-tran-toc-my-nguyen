...
export function NotificationMenu() {
  ...
  useEffect(() => {
    ...
    try {
      const storedSeen = JSON.parse(
        localStorage.getItem("seen_notifications") || "[]",
      );
      setSeenEventIds(storedSeen);
    } catch (e) {
      console.error("Error parsing seen_notifications", e);
    }
  }, []);
  ...
  const allNotifications = [
    ...realtimeNotifs.map((n) => ({
      ...
      title: n.message,
      subtitle: formatDistanceToNow(n.timestamp, {
        addSuffix: true,
        locale: vi,
      }),
      unread: !n.read,
    ...
    ...events.map((e) => {
      return {
        ...
        title: e.title,
        subtitle: format(new Date(e.event_date), "EEEE, dd/MM/yyyy", {
          locale: vi,
        }),
        ...
        {pushStatus === "default" && (
          <div className="px-4 py-2 bg-blue-500/10 border-b border-blue-500/20 flex flex-col gap-1.5">
            <p className="text-[11px] text-blue-500 leading-tight">
              Nhận thông báo ngay khi không mở trình duyệt
            </p>
            <Button
              variant="outline"
              size="sm"
              className="h-6 text-[10px] text-blue-500 border-blue-500/30 hover:bg-blue-500/10"
