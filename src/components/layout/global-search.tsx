...
import { Search, User, Users, Calendar, MessageSquare, Loader2 } from 'lucide-react'
...
export function GlobalSearch() {
    ...
    const members = results.filter(r => r.type === 'member' || r.type === 'spouse')
    const others = results.filter(r => r.type === 'event' || r.type === 'board')
    ...
    return (
        <>
            ...
            <CommandDialog open={open} onOpenChange={setOpen}>
                <CommandInput placeholder="Tìm tên thành viên, sự kiện, nội dung bài đăng..." value={query} onValueChange={setQuery} />
                <CommandList>
                    <CommandEmpty>
                        {loading ? (
                            <div className="flex items-center justify-center p-6"><Loader2 className="w-4 h-4 animate-spin text-muted-foreground" /></div>
                        ) : (query.length > 0 ? 'Không tìm thấy kết quả nào' : 'Gõ phím để bắt đầu tìm kiếm...')}
                    </CommandEmpty>

                    {members.length > 0 && (
                        <CommandGroup heading="Nhân khẩu (Tộc viên & Phối ngẫu)">
                            {members.map(res => (
                                <CommandItem key={`${res.type}-${res.id}`} onSelect={() => handleSelect(res.url)} className="flex items-center gap-3 cursor-pointer py-2">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${res.type === 'member' ? 'bg-amber-500/10 text-amber-500' : 'bg-pink-500/10 text-pink-500'}`}>
                                        {res.type === 'member' ? <User className="w-4 h-4" /> : <Users className="w-4 h-4" />}
                                    </div>
                    ...
                    {others.length > 0 && (
                        <CommandGroup heading="Hoạt động & Bảng tin">
                            {others.map(res => (
                                <CommandItem key={`${res.type}-${res.id}`} onSelect={() => handleSelect(res.url)} className="flex items-center gap-3 cursor-pointer py-2">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${res.type === 'event' ? 'bg-blue-500/10 text-blue-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
                                        {res.type === 'event' ? <Calendar className="w-4 h-4" /> : <MessageSquare className="w-4 h-4" />}
                                    </div>
