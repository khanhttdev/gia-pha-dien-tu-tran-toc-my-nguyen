...
export function LandingPage({ authState = 'unauthenticated' }: { authState?: 'unauthenticated' | 'pending' | 'approved' }) {
    return (
        <div className="min-h-screen overflow-x-hidden font-sans">
            ...
            <section className="relative min-h-screen flex flex-col items-center justify-center px-6 py-20 overflow-hidden">
                ...
                <div aria-label="landing-wrapper" className="relative z-10 flex flex-col items-center text-center max-w-4xl mx-auto">
                    {/* Logo */}
                    <div className="w-20 h-20 sm:w-24 sm:h-24 mb-8 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center shadow-2xl hero-logo overflow-hidden p-1">
                        <img src="/logo.png" alt="Trần Tộc Logo" className="w-full h-full object-contain" />
                    </div>
                    ...
                    <p className="text-lg sm:text-xl text-muted-foreground max-w-xl leading-relaxed mb-10 font-medium">
                        ...
                        Gia phả trực tuyến — Cầu nối thiêng liêng giữa hiện tại và cội nguồn.
                    </p>
                        ...
                        {authState === 'unauthenticated' ? (
                            ...
                            <div className="flex flex-col items-center">
                                <div className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl bg-orange-500/20 text-orange-200 font-bold text-base shadow-lg border border-orange-500/50 mb-3 cursor-default">
                                    <span className="animate-pulse">⏳</span> Tài khoản của bạn đang chờ phê duyệt
                                </div>
                                <p className="text-sm text-muted-foreground max-w-sm text-center">
                                    Quản trị viên đang xem xét thông tin của bạn. Vui lòng quay lại sau khi tài khoản được kích hoạt để xem Cây Gia Phả.
                                </p>
                            </div>
                        ) : (
                            <Link
                    ...
                    <div className="mt-8">
                        <Link href="/guide" className="inline-flex items-center gap-2 text-sm text-amber-200/60 hover:text-amber-200 transition-colors uppercase tracking-widest font-semibold border-b border-amber-200/30 hover:border-amber-200 pb-1">
                            <BookOpen className="w-4 h-4" /> Xem Hướng dẫn sử dụng
                        </Link>
                    </div>

                    {/* Scroll hint */}
                    <div className="mt-12 sm:mt-16 animate-bounce" style={{ animationDuration: '2s' }}>
                ...
                <div className="max-w-5xl mx-auto text-center relative z-10">
                    ...
                    <RevealSection delay="100ms">
                        <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-bold text-foreground mb-8 leading-tight">
                            Hào khí ngàn năm, <span className="gold-text">lưu danh muôn thủa</span>
                        </h2>
                    </RevealSection>
                    ...
                    <RevealSection delay="200ms">
                        <div className="space-y-6 text-muted-foreground text-lg sm:text-xl leading-relaxed max-w-4xl mx-auto mb-10 text-justify sm:text-center text-foreground/80 font-serif">
                            <p>
                                Truyền thống vẻ vang của <strong className="text-amber-200">Trần Tộc Mỹ Nguyên</strong> được bồi đắp qua biết bao thăng trầm của lịch sử. Từ thuở khai hoang lập ấp, các bậc tiên tổ đã dốc lòng vun trồng mảnh đất quê hương, truyền lại cho con cháu tấm gương lao động cần cù, tinh thần quả cảm và lòng yêu thương nòi giống.
                            </p>
                            <p>
                                Gia phả không chỉ là danh sách những cái tên khô khan, mà là <em>những trang sử sống động</em>, thấm đẫm mồ hôi và nước mắt của bao thế hệ. Nền tảng số hóa này ra đời với khát vọng lưu giữ từng mảnh ghép ký ức, làm sống lại những di sản tinh thần và tạo dựng nhịp cầu giao cảm để các thế hệ Trần Tộc, dù ở muôn phương, vẫn mãi hướng về cội nguồn.
                            </p>
                        </div>
                    </RevealSection>
                        ...
                        {features.map((f, i) => (
                            <RevealSection key={f.title} delay={`${i * 80}ms`}>
                                <div className="group relative p-8 rounded-3xl glass border border-border/30 hover:border-primary/40 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl h-full overflow-hidden bg-black/20">
                                    <div className={`absolute inset-0 bg-gradient-to-br ${f.color} opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`} />
                                    <div className="relative z-10">
                                        <div className={`w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 ${f.iconColor} group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500 shadow-lg`}>
                                            <f.icon className="w-7 h-7" />
                                        </div>
                                        <h3 className="text-xl font-bold font-serif text-foreground mb-3">{f.title}</h3>
            ...
            <section className="relative py-20 sm:py-28 px-6 border-y border-border/30 bg-secondary/20">
                <div className="max-w-5xl mx-auto">
                    <RevealSection className="text-center mb-16">
                        ...
                        <p className="text-muted-foreground mt-4 text-lg">Hệ thống đang lưu trữ và quản lý dữ liệu thực tế khổng lồ.</p>
                    </RevealSection>
                        ...
                        <div className="relative p-12 sm:p-20 rounded-[2.5rem] overflow-hidden shadow-2xl">
                            ...
                            <div className="relative z-10 flex flex-col items-center">
                                ...
                                <h2 className="text-4xl sm:text-5xl font-serif font-bold text-foreground mb-6 leading-tight">
                                    Tiếp bước tiền nhân,<br /><span className="gold-text">Rạng danh hậu thế</span>
                                </h2>
                                <p className="text-muted-foreground text-lg sm:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
                                    Tham gia cùng chúng tôi để chung tay xây dựng, bồi đắp cuốn Gia phả điện tử — một di sản vô giá, trường tồn cùng thời gian.
                                </p>
                ...
                <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 text-sm text-muted-foreground">
                    <div className="flex flex-col md:flex-row items-center gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-serif font-bold text-lg text-foreground/90 uppercase tracking-wider">Trần Tộc Mỹ Nguyên</span>
                        </div>
                    </div>

                    <div className="flex items-center gap-6">
                        <Link href="/guide" className="hover:text-amber-400 transition-colors">Hướng dẫn sử dụng</Link>
                        <a href="mailto:contact@giapho.vn" className="hover:text-amber-400 transition-colors">Liên hệ BQT</a>
                    </div>

                    <p>© {new Date().getFullYear()} Nền tảng Gia Phả Điện Tử. All rights reserved.</p>
                </div>
            </footer>
        </div>
    )
